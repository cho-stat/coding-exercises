...
def is_balanced(tree_root):
    ...
    while len(child_set) > 0:
        next_child_set = set()
        for root in child_set: 
            if root.left is None and root.right is None and min_depth == 0: 
                min_depth = current_depth 
            if root.left is not None:
                next_child_set.add(root.left)
            if root.right is not None: 
                next_child_set.add(root.right)
            
        if len(next_child_set) > 0:
            current_depth += 1
    ...
    return True if max_depth - min_depth <= 1 else False
